...
public class CChecklistItemData : CData
{
    ...
    /// <summary>
    /// get the default tS state id
    /// </summary>
    /// <param name="dsTS"></param>
    /// <param name="lStateID"></param>
    /// <returns></returns>
    public long GetTSDefaultStateID(DataSet dsTS, k_STATE_ID lStateID)
    {
        long lTSID = -1;

        foreach (DataTable table in dsTS.Tables)
        {
            foreach (DataRow dr in table.Rows)
            {
                ...
                if (lSID == (long)lStateID)
                {
                    if (lIsDefault == 1)
                    {
                        lTSID = CDataUtils.GetDSLongValue(dr, "TS_ID");
                        return lTSID;
                    }
                }
            }
    ...
    /// <summary>
    /// get the default OS state id
    /// </summary>
    /// <param name="dsOS"></param>
    /// <param name="lStateID"></param>
    /// <returns></returns>
    public long GetOSDefaultStateID(DataSet dsOS, k_STATE_ID lStateID)
    {
        long lOSID = -1;

        foreach (DataTable table in dsOS.Tables)
        {
            foreach (DataRow dr in table.Rows)
            {
                long lSID = CDataUtils.GetDSLongValue(dr, "STATE_ID");
                ...
                if (lSID == (long)lStateID)
                {
                    if (lIsDefault == 1)
                    {
                        lOSID = CDataUtils.GetDSLongValue(dr, "OS_ID");
                        return lOSID;
                    }
                }
            }
